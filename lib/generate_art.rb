...
def build_svg(item, icons)
  ...
  <<~SVG
    <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 #{WIDTH} #{HEIGHT}" width="#{WIDTH}" height="#{HEIGHT}" role="img" aria-label="#{escape(item['name'])}">
      ...
      <circle cx="#{blob_x}" cy="#{blob_y}" r="#{radius}" fill="#ffffff" opacity="0.06"/>
      <circle cx="#{WIDTH - blob_x / 2}" cy="#{HEIGHT - blob_y / 3}" r="#{radius / 2}" fill="#000000" opacity="0.08"/>
    ...
    #{lines.each_with_index.map { |line, i| %(    <text x="40" y="#{text_y + i * 30}" font-size="25" font-weight="700">#{escape(line)}</text>) }.join("\n")}
        <text x="40" y="#{text_y + lines.length * 30 + 6}" font-size="15" font-weight="600" opacity="0.72" letter-spacing="1.5">#{escape(item['sku'])}</text>
      </g>
    </svg>
  SVG
end
...
items.each do |item|
  ...
  File.write(path, build_svg(item, icons))
  written += 1
